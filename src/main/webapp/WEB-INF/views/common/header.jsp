...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark">
	<div class="container-fluid">
		<a class="navbar-brand" href="/springframework"> <img
			src="${pageContext.request.contextPath}/resources/image/logo-spring.png"
			alt="Avatar Logo" style="width: 40px;" class="rounded-pill"> <span
			class="align-middle">전자정부프레임워크(SpringFramework)</span>
		</a>
		<div>
			...
			<sec:authorize access="isAuthenticated()">
				...
				<%-- 사이트간 요청 위조 방지 설정이 비활성화되어 있을 경우 GET 방식으로 요청이 가능하다. --%>
				<a href="${pageContext.request.contextPath}/logout"
					class="btn btn-success btn-sm">로그아웃</a>

				<%-- 사이트간 요청 위조 방지 설정이 활성화되어 있을 경우 POST 방식으로만 요청이 가능하다. --%>
				<%-- <form method="post"
					action="${pageContext.request.contextPath}/logout">
					<input type="hidden" name="${_scrf.parameterName}"
						value="${_csrf.token}" />
