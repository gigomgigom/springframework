...
public class Ch09Controller {
	...
	public String fileUpload(String title, String desc, MultipartFile attach) throws Exception, IOException {
		log.info("제목 : " + title);
		log.info("설명 : " + desc);
		//파일의 정보
		log.info("원래 파일 이름(사용자가 선택한 파일 이름) : " + attach.getOriginalFilename());
	...
	@PostMapping("/fileUpload")
	public String fileUpload(Ch09FileUploadForm form) throws Exception, IOException {
		log.info("제목 : " + form.getTitle());
		log.info("설명 : " + form.getDesc());
		//파일의 정보
		log.info("원래 파일 이름(사용자가 선택한 파일 이름) : " + form.getAttach().getOriginalFilename());
	...
	@ResponseBody
	public String fileUploadAjax(Ch09FileUploadForm form) throws Exception, IOException {
		log.info("제목 : " + form.getTitle());
		log.info("설명 : " + form.getDesc());
		//파일의 정보
		log.info("원래 파일 이름(사용자가 선택한 파일 이름) : " + form.getAttach().getOriginalFilename());
		...
		jsonObject.put("savedFileName", destFile.getName());
	...
	public void downloadFile(String fileName, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//한글로 되어있는 파일이름을 => ISO-8859-1 문자셋으로 구성된 파일 이름으로 변환
